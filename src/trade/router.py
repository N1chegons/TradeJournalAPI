...
from src.trade.schemas import TradeCreate, TradeUpdate
...
# patch
@router.patch("/update_trade/{trade_id}/", summary="Update trade with ID.")
async def update_trade(trade_id: int, schemas: TradeUpdate = Depends(),  user: User = Depends(cur_user)):
    updated_trade = await  TradeRepository.update_trade_in_journal(trade_id, schemas, user)
